...
public class ImgSwiperAdapterOrderDetail extends PagerAdapter {
    ...
    private ViewSellOrderDetailActivity _activity;
    ...
    public ImgSwiperAdapterOrderDetail(ViewSellOrderDetailActivity activity,
                                       ViewPager viewPager, ArrayList<ProductImagesOrd> productImagesList) {
...
//        mImaView.setImageBitmap(bitmapList.get(position));
        ...
        ProductImagesOrd myItem = this.productImagesList.get(position);

        Picasso.with(_activity).load( myItem.image).into(mImaView);

        //Uri uri = Uri.parse(ContentResolver.SCHEME_ANDROID_RESOURCE + "://" + _activity.getPackageName() + "/drawable/" + "dummy_user");
       // Picasso.with(_activity).load(uri).into(mImaView);
